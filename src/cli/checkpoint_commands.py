...
def delete(
    ...
    try:
        ...
        if deleted_count > 0:
            console.print(
                f"✅ Deleted {deleted_count} checkpoint(s) for thread: {thread_id}", style="green"
            )
        else:
...
def prune_old(
    ...
    try:
                    ...
                    try:
                        ...
                        if ts < cutoff:
                            checkpoints_to_delete.append(
                                (thread_id, checkpoint_tuple.checkpoint["id"])
                            )
                    except Exception as e:
        ...
        if deleted_count > 0:
            console.print(
                f"✅ Pruned {deleted_count} checkpoint(s) older than {days} days", style="green"
            )
        else:
